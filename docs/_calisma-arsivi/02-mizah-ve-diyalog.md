...
- Bu dokümanda geçen tek hadis: **"Temizlik imanın yarısıdır"** (Müslim, Tahâret 1) — sahih; `saka_talk` içinde ciddi tonda, espriden ayrı cümlede kullanılır ve kodeks girişinde kaynağı yazılır. Yeni hadis/ayet eklemek bu dokümanın yetkisinde DEĞİLDİR; ekleme ancak kaynak denetiminden geçmiş ayrı içerik kararıyla olur.
...
saka_talk: {
  ...
  choices: [
    ...
    },
    {
      ...
      action: () => ({
        text: `"Hor görme beyim; susuza su vermek sadakaların makbulündendir. Ecdat yol boylarına sebiller kurmuş: parasız, hesapsız — karşılığı yalnız dua. Orduda dahi saka neferi vardır; gazi susuz kalmasın diye kırba sırtlanır. Hem Peygamber Efendimiz 'Temizlik imanın yarısıdır' buyurmuştur. Bizimkisi işte o yolun hamallığıdır."`,
        choices: [
          {
...
guard_talk: (() => {
  const openers = [
    `"Dur! Kim var?.. Tamam- tanıdım beyim, geç. Yüzünden değil, atından tanıdım; at senden meşhur."`,
...
- [ ] Din adamı/ibadet/kutsal kavram hiçbir mizah verisinde geçmiyor (manuel okuma + `imam|molla|namaz|ezan|ayet|hadis|mescid|zemzem|günah|melek` grep'inin mizah havuzlarında sıfır eşleşmesi; `saka_talk` içindeki hadis cümlesi tek istisnadır ve ciddi dalda yer alır).
- [ ] Yeni hiçbir replik oyuncuya sabit isimle hitap etmiyor (`Murad` grep'i yeni içerikte sıfır).
- [ ] Karakter sınırları (Bölüm 1.2 tablosu) aşılmıyor (göz kontrolü; istenirse basit script).
