...
**Ek uyum notları (çelişki değil, netleştirme):**
...
|---|---|---|---|---|
| **0** | Depo Güvenliği + İlk Saati Kurtar | İlk 60 dakika "bozuk oyun" hissi vermiyor: görevler ilerliyor, pusula doğru, bildirimler görünüyor, sefer mühürlü, 4 sessiz NPC konuşuyor, hukuki riskler kapalı | 24-28 | 3-4 |
...
| **2** | Çekirdek Döngü: Zaman + Ekonomi + Kayıt | 1 gün ≈ 16,5 dk; 1396 gerçek takvimi + atlama kartları; uyku; gider döngüsü; otomatik kayıt + Akşam Hesabı + Vakayiname | 58-74 | 8-10 |
| **3** | Niğbolu ve Kampanya | 5 safhalı metin-taktik Niğbolu; gerçek talim; sefer hazırlık defteri; görev fiil çeşitliliği; Ali dram zinciri canlı | 62-86 | 9-12 |
...
| **5** | Cila ve Dağıtım | Performans (hitch'siz), erişilebilirlik ilk paketi, çevrimdışı bütünlük, dokümantasyon, KPI playtest turu, yayın kapısı | 42-56 | 6-8 |
| | **TOPLAM** | | **340-451** | **48-63 iş günü** |
...
**Amaç:** Oyunun ilk 60 dakikasında oyuncuya "bozuk" hissi veren yedi cerrahi hatayı kapatmak, sürüm kontrolünü kurmak (şu an depo git deposu DEĞİL — her düzeltme geri alınamaz risk taşıyor), hukuki/marka risklerini (Stan Lee modeli, M&B/KCD ibaresi, sahte Steam vaadi) dağıtımdan bağımsız olarak hemen temizlemek ve dört sessiz NPC'yi (saka + 3 nöbetçi) hazır içerikle konuşturmak. Bu faz bitmeden hiçbir içerik/özellik işine başlanmaz (05-teknik P0 kuralı). Faz sonunda oyun: görevleri ilerleyen, pusulası doğru, bildirimi görünen, seferi mühürlü, hukuken temiz bir demo hâline gelir.

### 3.1 İş listesi

| ID | Başlık | Dosya referansları | Tarif | Dayanak | Süre | Bağımlılık |
|---|---|---|---|---|---|---|
| F0-01 | git init + baseline + tasarım dokümanlarını depoya al | depo kökü; `docs/design/` | `git init`; `.gitignore` (dist/, node_modules/); ilk commit "baseline: 97/97 tests"; `dist/` takipten çıkar. Beş tasarım dokümanı + bu doküman `docs/design/01..06-*.md` olarak depoya kopyalanır (geliştiricinin tek başvuru kaynağı repo içinde olur). | 05-teknik P2-50, §10 sıra-0 | 1 | — (HER ŞEYDEN ÖNCE) |
...
| F0-06 | Sefer butonuna quest_campaign kapısı | `UIManager.js:340-364`, `HistoryEventSystem.js:12-36` | `joinActiveCampaign` başına kapı: quest_campaign aktif değilse `false` + "Sultanın fermanı henüz sana ulaşmadı" bildirimi; buton `disabled` + neden metni. Buton GİZLENMEZ — mühürlü gösterilir (kampanya hedefini ilk dakikadan pazarlar). | 05-teknik P0-5; 01-akış B3, §3.2 | 1,5 | F0-01 |
...
| F0-13 | Electron port düzeltmesi | `electron-main.cjs:39-53`, `vite.config.js:6`, `docs/DEVELOPMENT_SPEC.md:5` | devUrl `http://localhost:3000`; 5173 yedeği tamamen kaldırılır (yanlış-uygulama-yükleme riski); doküman port referansları güncellenir. | 05-teknik P1-22, §9.1 | 0,5 | F0-01 |

**Faz toplamı: ≈ 27 saat (24-28 bandı) ≈ 3-4 iş günü.**
...
- [ ] `git log` baseline commit'i gösteriyor; `git status` temiz; `dist/` ve `node_modules/` izlenmiyor; `docs/design/` altında 6 tasarım dokümanı var.
...
- [ ] `git grep -i "stanlee\|stan lee"` → 0; `grep -i "mount\|kingdom" index.html README.md` → 0; Koca Yakub prosedürel yaşlı-bilge model olarak görünüyor ve diyalog/AI davranışı değişmedi; `public/models` ≥ 29MB küçüldü; `docs/ASSETS.md` mevcut ve her `public/` varlığının satırı açılmış (Karar sütunu Faz 5'e kadar "İNCELEMEDE" olabilir).
...
- [ ] **Juice smoke testi (01-akış §4):** 1 arzuhal kabulü + 1 kılıç isabeti + 1 görev tamamlama kaydında her olay ≥2 duyusal kanaldan (görsel+ses) yanıt veriyor; kılıç isabetinde hasar+ses+sarsıntı+parçacık AYNI karede (frame log'da fark = 0 kare) ve hit-stop 40-80 ms (01-akış K7).
...
| F2-11 | Oturum ritüeli: Akşam Hesabı + Kaldığın Yer + Vakayiname | `UIManager.js`, `index.html`, `GameState.js:207-209` | (a) Akşam Hesabı tam ekran kartı (yatsı/uykuda): kasa giriş-çıkış + itibar okları + vazife özeti/yarın işleri + 1 satır söylenti VEYA tarih vinyeti (dönüşümlü). (b) Kaldığın Yer kartı (yüklemede): tarih + konum + aktif hedef + son 3 olay + anlık değerler + saatli tehdit + "Devam et" (yükleme→oynanış ≤30 sn). (c) Vakayiname: J modalına 2. sekme; her `addNotification` günlüğe `{gün, vakit, tür, metin}` olarak yazılır (tavan 200, FIFO, kayda girer — Ç8); filtre çipleri (Vazife/Tımar/Sefer/Kodeks); gün başlıkları çift takvimli. | 01-akış §5.2-5.4; 05-teknik P2-29 | 9 | F2-02, F2-10 |
...
**Faz toplamı: ≈ 68 saat (58-74 bandı) ≈ 8-10 iş günü.**
**Faz içi kritik yol:** F2-01 → F2-02 → (F2-04, F2-05, F2-09) → F2-10 → F2-11. F2-06, F2-05'e bağlı; F2-12/13 paralel.
...
- [ ] **Ekonomi:** vergi hasat penceresi dışında pasif + tooltip; aynı yıl ikinci tahsilat imkânsız; 3 saatlik bot koşusunda kasa eğrisi monoton artmıyor (≥2 net-negatif hafta, 01-akış K11); aynı arzuhal yapısı iki kez inşa edilemiyor; nafaka ödenemeyince akçe 0'ın altına inmiyor, bölük sadakati düşüyor.
...
| ID | Başlık | Dosya referansları | Tarif | Dayanak | Süre | Bağımlılık |
|---|---|---|---|---|---|---|
| F3-01 | CampaignBattleSystem'i bağla (kanonik Niğbolu) | `CampaignBattleSystem.js:26-187`, `HistoryEventSystem.js:9-81`, `UIManager.js:346,358,824-834` | `joinActiveCampaign` başarılı olunca `startNicopolisBattle()`; savaş sonucu modalı safha metni + 2-4 seçenek butonu render eden döngüye genişletilir (`executePhaseAction` durum tutuyor); `concludeBattle` ödül/`legSevered` zincirini işletir; `simulateNigboluCampaign` ve `rewardAkce/rewardRep` çelişkisi temizlenir (ödül tek kaynak: CampaignBattleSystem sonuç tablosu); safha aralarında otomatik checkpoint kaydı (F2-10 tetik listesine eklenir); sefer girişinde `flags.inCampaignScene=true`, köye dönüş sahnesinde temizlenir (02-mizah §6 bayrak-4 sözleşmesi). Final tetiği: quest_campaign aktif + g178. | 05-teknik P1-06; TARIHSEL §13 Aşama 4 köprüsü; 02-mizah §6 | 10 | F2 |
| F3-02 | Görev kapıları: Ali seçenek gating + activate/progress ayrımı | `DialogueSystem.js:117-131,223-236,472-484`, `QuestSystem.js:439-441` | (a) İmam/demirci/attar'daki Ali seçenekleri yalnız `aliStatus.legSevered===true` VE `quest_save_ali_leg` aktifken listelenir (choices runtime filtresi). (b) `advanceObjective` kilitli görevi aktive edemez — aktifleştirme yalnız `syncAvailableQuests`'in işi. | 05-teknik P1-12, P1-13; 01-akış B14; 04-islam §4.2/4 (A-K4) | 3 | F3-01 |
...
| F3-07 | SupplySystem'i bağla: Sefer Hazırlık Defteri + sefer sepeti + aşınma | `SupplySystem.js:39-123`, `UIManager.js` (HUD göstergesi), `ArcherySystem.js` (ok stoğu), `CombatSystem.js` (durability), `TimarSystem.js` | HUD'a "Sefer Hazırlık Defteri" göstergesi: cebelü sayısı/talim derecesi, erzak, ok, nal, at kondisyonu, yoklama notu; ok stoğu ArcherySystem'e, `reduceDurability` dövüşe bağlanır (her 25 isabetli vuruşta bileme ihtiyacı, 20-30 akçe); sefer sepeti: erzak 250 + ok/nal 120 + araba 100 ≈ 470 taban, memnun haneler gönüllü katkısıyla −%40'a kadar (hane memnuniyeti Perde I kararlarından); hazırlık kalemleri F3-01 safhalarında ölçülebilir avantaja çevrilir (ör. ok stoğu 2. safha yaylım sayısı). | 05-teknik §2 TUT-BAĞLA; 01-akış Döngü D, §7.3; TARIHSEL §13 Aşama 2 | 10 | F3-01, F3-06 |
| F3-08 | Yeni hedef tipleri + Perde I-II görev varyantları | `QuestSystem.js:16-401,439-511`, `DialogueSystem.js`, `TownGenerator.js` (etkileşim noktaları) | `advanceObjective`'e yeni hedef tipleri: `inspect` (E ile nokta inceleme), `carry` (fiziksel taşıma: yürüyüş yavaş, kılıç kapalı), `track` (iz noktaları zinciri), `seal` (mühürleme mini-ekranı). 01-akış §6 tablosundaki varyantlar Perde I-II görevlerine uygulanır: quest_inspect (3 hane ziyareti, incele×3), quest_water_dispute (ark incele + sınır taşı iz sür + 3 çözüm yolu), quest_blacksmith (kömür taşı VEYA öde), quest_imam (cuma zaman kapısı — fail YOK), quest_attar (kantaron topla VEYA satın al), quest_dede_flag (aktif hatırlama 2 soru), quest_bandits (önce iz sür; öldür/yakala/teslim-al yolları). Görev işaretçisi bölge gösterir (30 m → 8 m daralma); diyalog seçeneklerinde rakam yok, niyet cümlesi var. | 01-akış §6; TARIHSEL §9.7; 05-teknik P1-13 zemini | 14 | F3-02 |
| F3-09 | Perde III-IV görev varyantları | `QuestSystem.js` (quest_neighbor, quest_castle, quest_campaign, quest_inn_spy, quest_save_ali_leg) | quest_neighbor: atlı eskort (kısa riskli / uzun güvenli yol); quest_castle: yoklama defteri mühürleme mini-ekranı (eksiği bildir / sakla — Rumeli'de sonuç); quest_inn_spy: belge karşılaştırma (iki tezkere incele, tarih çelişkisi; yanlış suçlama oyunu kilitlemez, tazmin görevi açar) — metinler Ç/F4-10'daki delil-temelli düzeltmeyle uyumlu yazılır; quest_campaign: 3 günlük hazırlık listesi (erzak taşı, ok/nal al, araba onar → sefer); quest_save_ali_leg: tetik F3-01 sonucuna bağlanır, merhem/değnek al-taşı zinciri. | 01-akış §6; 03-tarih §4.2 (delil ilkesi) | 10 | F3-08, F3-01 |
...
**Faz toplamı: ≈ 75 saat (62-86 bandı) ≈ 9-12 iş günü.**
...
- [ ] HUD'da Sefer Hazırlık Defteri görünüyor; hazırlık kalemlerinden ≥5'i Niğbolu safhalarında ölçülebilir fark üretiyor: düşük/yüksek hazırlıklı iki otomatik koşuda sonuç farkı var (01-akış K12/D kabulü).
...
- [ ] R1-R5 (Ramazan/bayram): g68'de "Ramazan 1" HUD'da; Ramazan gündüz EATING yok, iftarda ≥8 NPC sofrada; bayram günlerinde WORKING regular yok; `gameDayToHijri` 4 çıpa aserti; iftara katılım stat değiştirmiyor.
...
| F5-03 | Chunk ayrıştırma + font yerelleştirme + çevrimdışı doğrulama | `vite.config.js`, `GeminiService.js`, `index.html:7-9`, `public/fonts/` | `manualChunks: {three:['three']}`; GeminiService dinamik `import()`; Cinzel/Amiri/Outfit woff2 yerel `@font-face` (OFL lisansları ASSETS.md'ye); ağ tamamen kapalıyken `npm run desktop` eksiksiz açılır (duman testi 1-7). Gemini kadı akışı çevrimdışı-öncelikli bağlandıysa (P1-05) doğrulanır; bağlanmadıysa BURADA tamamlanır (heuristic varsayılan, x-goog-api-key header, opsiyonel anahtar alanı). | 05-teknik §5.5, P2-40, P1-05, §9.3 | 8 | F5-01 |
| F5-04 | Dünya cilası paketi | `TownGenerator.js:243-257,290,682-695,26-32`, `Player.js:257-264`, `VillagerAI.js:164-184`, `NPCManager.js:643-651`, `ModelBuilder.js:1190`, `TextureGenerator.js:11-19` | Kale yolu meşaleleri (her 6. plaka, paylaşımlı ışık); köprü geçilebilir + nehir yasak bölge (P1-17); `getTerrainHeight` düz zemine sabitlenir (havada yürüme biter); NPC/harami collider paylaşılan AABB yardımcıyla; at bacak salınımı; talimgâh hedef tahtaları archeryTargets'a; anisotropy dokulara. | 05-teknik P1-17, P2-15/16/17/19/21/24 | 10 | F5-01 |
...
| F5-09 | KPI playtest turu + yayın kapısı | tüm oyun; `docs/design/` | 01-akış §8 protokolü TAM tur: soğuk başlangıç (n≥3), 3 saatlik bot koşusu, içerik denetimi (A/B/C/R + 18.1 + AUDIT), regresyon. 18 KPI tablosu tek tek ölçülür ve sonuç raporu `docs/design/kpi-raporu.md`'ye yazılır. Yayın kapısı kontrol listesi: duman testi 10/10 + KPI hedefleri + Ç11 ezan şartı + ASSETS.md kapanışı + dış denetimler (tarih + ilahiyat) kapanmış. Steam kararı AYRI faz olarak ertelenmiş kalır (05 §3.4). | 01-akış §8; 05-teknik §7.5 | 8 | F5-01..08 |

**Faz toplamı: ≈ 58 saat (42-56 bandı üst sınırıyla; KPI düzeltmeleri dahil ≈ 6-8 iş günü).**
...
3. Yayın kapısı checklist'inin kendisi `docs/design/kpi-raporu.md`'de doldurulmuş tablo olarak teslim edilir (denetçi belgeyi kontrol eder).
...
|---|---|---|---|
| Faz 0 | 24-28 | 3-4 | 3-4 gün |
| Faz 1 | 34-42 | 5-6 | 8-10 gün |
| Faz 2 | 58-74 | 8-10 | 16-20 gün |
| Faz 3 | 62-86 | 9-12 | 25-32 gün |
| Faz 4 (altyapı+4A+4B+4C) | 120-165 | 17-23 | 42-55 gün |
| Faz 5 | 42-58 | 6-8 | 48-63 gün |
| **TOPLAM** | **340-453** | **48-63** | — |
...
| 3-4 | Faz 2 (F2-01/02 ilk — takvim kararı kilitlensin) |
| 5-7 | Faz 3 (F3-01 ilk; talim/supply paralel) |
| 8 | Faz 4 altyapı (F4-01/02) + ezan kaydı SİPARİŞİ + ilahiyat/tarih dış denetim randevuları |
| 9 | 4A Mizah (bağımsız teslim + kabul) |
| 10-11 | 4B Tarih (kodeks → havadis → doğruluk → çevresel) |
| 12-13 | 4C İslami (namaz/ezan → cuma → zekât → Ramazan → İlmihal + AUDIT) |
| 14-15 | Faz 5 (perf → çevrimdışı → cila → KPI turu) |
| 16 | Tampon + dış denetim düzeltmeleri + yayın kapısı |

**Toplam: ~16 hafta (≈4 ay).** Erken görünürlük kilometre taşları: Hafta 2 sonu "ilk saat düzgün" demosu; Hafta 4 sonu "akıcı gün + kayıt" demosu; Hafta 7 sonu "uçtan uca kampanya" demosu; Hafta 11 sonu "işveren hedefleri görünür" demosu.

**Kapsam esnekleme kuralı (takvim daralırsa):** Kesilebilir ilk kalemler sırasıyla — F3-09'un iki görev varyantı (quest_neighbor eskortu, quest_castle mini-ekranı basit diyaloga düşer), F4-06 v1'in örs/meydan kanalları (yalnız hamam kalır), F5-04'ün at animasyonu ve anisotropy. KESİLEMEZ olanlar: tüm Faz 0-2, F3-01, F4-08, F4-13, D1-D12 kuralları, tüm dinî-hassasiyet işleri.
