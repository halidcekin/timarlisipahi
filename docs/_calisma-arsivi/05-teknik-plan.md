...
| P1-04 | `src/core/GameState.js:263-268,271-285` | `checkHistoricalEvents` yıl artırıldıktan SONRA çağrıldığı için `year === 1396` koşulu asla sağlanmıyor; yıl bazlı ferman/sefer üretimi ölü. | `checkHistoricalEvents()` çağrısını `this.time.year++` satırının ÖNCESİNE al ve fonksiyona "biten yıl" semantiği ver (veya koşulları `year === 1397` gibi yeni-yıl değerlerine çevir — birini seç, yorum satırıyla belgele). Yıl→olay eşlemesini switch yerine `src/data/historicalEvents.js` tablosundan oku (bkz. §4). | 1-2s | Test: `time.year=1396; time.seasonIndex=3; advanceSeason()` → activeCampaign/ferman bildirimi üretildi assert. |
| P1-05 | `src/ui/UIManager.js:277-283,587-681`, `src/services/GeminiService.js:6-12,43` | ⊕ Oyunun en özgün mekaniği ölü: "Reddet" butonu `openRejectionModal` yerine düz `rejectPetition` (-5 moral) çağırıyor; `setApiKey`'i çağıran UI yok; API anahtarı URL query'de taşınıyor. | (a) `:277` handler'ı `openRejectionModal()`'a yönlendir; modal akışı (yaz → `submitRejectionReason` → `showKadiVerdict`) zaten hazır. (b) Varsayılan yol **çevrimdışı `evaluateHeuristic`** olur (TARIHSEL §15: ana görev çevrimiçi hizmete bağlanamaz); API anahtarı girilmişse Gemini denenir, hata/timeout'ta sessizce heuristic'e düşer. (c) `callGeminiAPI`'de anahtarı `?key=` yerine `x-goog-api-key` header'ı ile gönder. (d) Ayarlar/başlangıç ekranına opsiyonel "Gemini API anahtarı" alanı (boş bırakılabilir). | 4-6s | Manuel: arzuhal reddet → gerekçe modalı açılır, gerekçe yaz → kadı hükmü modalı, asayiş/moral gerekçe kalitesine göre değişir; ağ kapalıyken akış aynen çalışır. Test: `evaluateHeuristic('trol metin')` düşük puan, meşru gerekçe yüksek puan assert. |
...
| P1-22 | `electron-main.cjs:39-53` ↔ `vite.config.js:6` | Electron önce 5173'ü deniyor, Vite 3000'de: her açılışta gecikme + makinede 5173'te başka Vite projesi varsa YANLIŞ uygulama yükleniyor. | §9.1'deki düzeltme: birincil URL `http://localhost:3000`, yedek dist. 5-dakikalık iş, P1'e alındı çünkü yanlış-uygulama-yükleme gerçek bir tehlike. | 15dk | Manuel: `npm run dev` + `npm run desktop` → doğru oyun açılıyor; 5173'te başka proje çalışırken de doğru oyun açılıyor. |
...
| P2-50 | Depo hijyeni: `dist/` çıktısı depoda; **dizin git deposu değil** | Build çıktısı kaynakla senkron kayması riski; sürüm kontrolü yok — her düzeltme geri alınamaz risk taşıyor. | `git init` + `.gitignore` (dist/, node_modules/) + ilk commit ("baseline: 97/97 tests"); dist/ takipten çıkar. Bu madde **her işten önce** yapılmalıdır (P0'dan da önce). | 30dk | `git log` baseline commit'i gösteriyor; `git status` temiz. |
...
**Süre:** 2-3s. **Doğrulama:** Manuel — Koca Yakub köy meydanında beyaz sakallı prosedürel model olarak görünüyor, diyalog/AI davranışı değişmedi; repo'da "stanlee" araması 0 sonuç; `public/models` boyutu ≥ 29MB küçüldü.
...
**Şimdi (30dk):**
1. `electron-preload.cjs`'e yorum bandı: `// steamworks.js is intentionally NOT a dependency yet — see docs/design/05-teknik-plan.md §3.4` (sessiz ölü özellik → belgeli bilinçli karar).
...
src/data/
  ...
  historicalEvents.js  // YIL→ferman/sefer tablosu (P1-04 tüketir) — gün-bazlı HistoricalNews'ten ayrıdır
...
// src/data/CodexData.js — 03-tarih §2.2 şeması + teknik ekler
{
  ...
  review: { history: false, islamic: false }          // içerik denetim onay bayrakları (§8 süreci)
}
```

`review.islamic` alanı İslami içerik taşıyan maddelerde Ehl-i Sünnet kontrolünden (04-İslami içerik dokümanı §5 protokolü) geçtiğini işaretler; her ikisi `true` olmayan madde **build'e girer ama oyunda "taslak" rozetiyle gizlenir** (CodexSystem'de basit filtre) — denetimsiz içerik oyuncuya sızmaz. `unlock` tetiklerinin çalıştırılması ve `unlockCodex` effect'i (§4.2) `src/systems/CodexSystem.js`'e (03-tarih §2.2'nin öngördüğü singleton) bağlanır.
...
8. **Tarih/din içerik kapısı:** İslami veya tarihsel bilgi taşıyan her içerik parçası `historicalConfidence` (A/B/C/R) taşır; İslami içerik Ehl-i Sünnet kontrol listesinden (04-içerik dokümanı) geçmeden `review.islamic:true` yapılamaz; din adamı/ibadet mizah nesnesi olamaz (şema düzeyinde test edilir, §4.4). Sistem anlatıcısı tarafsız dil kullanır ("Haçlı ordusu"; "küffar" yalnız karakter ağzında — TARIHSEL §11).
...
10. **Sürüm kontrolü:** P2-50 ile git kurulur; her backlog kalemi ayrı commit; davranış değiştiren commit'ler geri alınabilir küçüklükte tutulur.
...
|---|---|---|---|---|
| 0 | Depo güvenliği | P2-50 (git init + baseline) | 30dk | `git log` baseline |
| 1 | P0 paketi | P0-1…P0-7 | 1,5-2g | Duman testi adım 1-5 + `npm test`/build yeşil |
| 2 | Hukuk paketi | §3.1, §3.3, §3.4-şimdi, §3.2 denetim başlangıcı | 1g | "stanlee/mount/kingdom" grep 0; ASSETS.md açıldı |
